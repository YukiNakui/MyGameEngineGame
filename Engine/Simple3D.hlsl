...
cbuffer global
{
    ...
    float4x4 matW; //法線をワールド座標に対応させる行列=回転*スケールの逆行列
...
VS_OUT VS(float4 pos : POSITION, float4 uv : TEXCOORD, float4 normal : NORMAL)
{
    ...
    outData.cos_alpha = clamp(dot(normal, light), 0, 1);//0～1の範囲で区切る（指定した範囲を越えると越えないように値を切ってくれる）
...
{
    float4 Id = { 1.0, 1.0, 1.0, 0.0 }; //光の強さ、通常全色1.0
