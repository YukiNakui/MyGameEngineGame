...
HRESULT Direct3D::Initialize(int winW, int winH, HWND hWnd)
{
	...
	//戻り値HRESULTの関数(Createなど)を使ったらエラー処理
...
HRESULT Direct3D::InitShader3D()
{
	...
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &shaderBundle[SHADER_TYPE::SHADER_3D].pVertexShader_);//失敗したらE_～ が帰ってくる
	...
	rdc.CullMode = D3D11_CULL_BACK; //多角形の裏側は描画しない(カリング) NONEですべての三角形を描画　FRONTで表側は描画しない
	rdc.FillMode = D3D11_FILL_SOLID; //多角形の内部を塗りつぶす(WIREFRAMEでワイヤフレーム表示)
	rdc.FrontCounterClockwise = false; //反時計回りを表にするかどうか（がfalseなので時計回りが表）CounterClockwise=反時計回り
...
HRESULT Direct3D::InitShader2D()
{
	...
	assert(pCompileVS != nullptr);
	hr = pDevice->CreateVertexShader(pCompileVS->GetBufferPointer(), pCompileVS->GetBufferSize(), NULL, &shaderBundle[SHADER_TYPE::SHADER_2D].pVertexShader_);//失敗したらE_～ が帰ってくる
...
{
	//背景の色
	float clearColor[4] = { 139 / 255.0f, 0 / 255.0f, 0 / 255.0f, 1.0f };//R,G,B,A
